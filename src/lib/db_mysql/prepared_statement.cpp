...
#ifdef _WIN32 // 5.7版本以下没有正确包含winsock头文件，5.7之后已修复
#include <winsock2.h>
#endif
#include <mysql.h>
#include <sstream>
...
{
	LOG_ERROR << "Attempted to bind parameter " << index + 1 << " on a PreparedStatement " << stmtIndex 
		<< " (statement has only " << paramCount << " parameters)";
	return false;
}
...
void MySQLPreparedStatement::checkValidIndex(uint8 index)
{
	...
	{
		LOG_WARN << "Prepared Statement (id: " << m_stmt->m_index << " trying to bind value on already bound index: " << index;
	}
}
