...
#include "lib/interface_header/singleton.hpp"
...
public:
	...
	template<SERVER_TYPES ServerType, typename T>
	bool launch(int argc, char* argv[])
	{
		ILib* lib = new T(this);
		return launch(lib, ServerType, argc, argv);
	}

	bool launch(ILib* lib, SERVER_TYPES server_type, int argc, char* argv[]);
	bool loop();
	void stop() override;
	void destruct();
