...
CoroutineManager::~CoroutineManager()
{
    for (int i = 0; i < coroutineList_.size(); i++)
    {
...
void CoroutineManager::resume(int id)
{
    ...
    if (id < 0 || id >= this->coroutineList_.size())
    {
        return;
    }
...
{
    if (id >= 0 && id < coroutineList_.size())
    {
...
{
    int id = 0;
    for (; id < this->coroutineList_.size(); ++id)
    {
    ...
    if (id == this->coroutineList_.size())
    {
