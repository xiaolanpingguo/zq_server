...
GameSSModule::GameSSModule(ILibManager* p)
{
	libManager_ = p;
}

bool GameSSModule::init()
{
...
bool GameSSModule::run()
{
	update();
	return true;
}

GameSSSessionPtr GameSSModule::getSessionByServerId(int server_id)
{
...
void GameSSModule::getSessionByServerType(int server_type, GameSSSessionList& vec)
{
	for (const auto& ref : socketMap_)
	{
