...
{
	if (_objectState.empty())
	{
		_errors.emplace_back("Deserializer::Key, _objectState is empty.");
		return false;
	}
	...
	if (!field)
	{
		_errors.push_back(fmt::format("Message {} has no field {}.", _objectState.top()->GetTypeName().c_str(), str));
		return false;
	}
...
bool Deserializer::Null()
{
	if (_state.empty())
	{
		_errors.emplace_back("Deserializer::Null, _state is empty.");
		return false;
	}
...
bool Deserializer::Bool(bool b)
{
	if (_state.empty())
	{
		_errors.emplace_back("Deserializer::Bool, _state is empty.");
		return false;
	}
...
bool Deserializer::Int(int32 i)
{
	if (_state.empty())
	{
		_errors.emplace_back("Deserializer::Int, _state is empty.");
		return false;
	}
...
bool Deserializer::Uint(uint32 i)
{
	if (_state.empty())
	{
		_errors.emplace_back("Deserializer::Uint, _state is empty.");
		return false;
	}
	...
	default:
		_errors.push_back(fmt::format("Expected field type to be uint32 or string but got {} instead.", _state.top()->cpp_type_name()));
		return false;
	}
...
bool Deserializer::Int64(int64 i)
{
	if (_state.empty())
	{
		_errors.emplace_back("Deserializer::Int64, _state is empty.");
		return false;
	}
...
bool Deserializer::Uint64(uint64 i)
{
	if (_state.empty())
	{
		_errors.emplace_back("Deserializer::Uint64, _state is empty.");
		return false;
	}
...
bool Deserializer::Double(double d)
{
	if (_state.empty())
	{
		_errors.emplace_back("Deserializer::Double, _state is empty.");
		return false;
	}
	...
	case google::protobuf::FieldDescriptor::CPPTYPE_DOUBLE:
		...
		break;
	default:
		_errors.push_back(fmt::format("Expected field type to be float or double but got {} instead.", _state.top()->cpp_type_name()));
		return false;
	}
...
bool Deserializer::String(const Ch* str, rapidjson::SizeType /*length*/, bool /*copy*/)
{
	if (_state.empty())
	{
		_errors.emplace_back("Deserializer::String, _state is empty.");
		return false;
	}
	...
	case google::protobuf::FieldDescriptor::CPPTYPE_ENUM:
	{
		...
		if (!enumValue)
		{
			_errors.push_back(fmt::format("Field {} enum {} does not have a value named {}.", field->full_name().c_str(), field->enum_type()->full_name().c_str(), str));
			return false;
		}
	...
	case google::protobuf::FieldDescriptor::CPPTYPE_STRING:
		...
		break;
	default:
		_errors.push_back(fmt::format("Expected field type to be string or enum but got {} instead.", _state.top()->cpp_type_name()));
		return false;
	}
...
bool Deserializer::StartObject()
{
	...
	if (!_state.empty())
	{
		...
		{
			_errors.push_back(fmt::format("Expected field {} to be a message but got {} instead.", _state.top()->cpp_type_name()));
			return false;
		}
...
bool Deserializer::StartArray()
{
	...
	{
		_errors.push_back(fmt::format("Expected field {} type to be exactly an array OR bytes but it was both or none.", _state.top()->full_name().c_str()));
		return false;
	}
...
bool Deserializer::CheckType(google::protobuf::FieldDescriptor::CppType expectedType)
{
	if (_state.empty())
	{
		_errors.push_back(fmt::format("CheckType, _state is empty, CppType:{}.", expectedType));
		return false;
	}

	if (_state.top()->cpp_type() != expectedType)
	{
		_errors.push_back(fmt::format("Expected field {} type to be {} but got {} instead.",
...
bool Deserializer::EndArray(rapidjson::SizeType /*memberCount*/)
{
	if (_state.empty())
	{
		_errors.emplace_back("Deserializer::EndArray, _state is empty.");
		return false;
	}
...
std::string ProtoJsonModule::serialize(const google::protobuf::Message& message)
{
...
bool ProtoJsonModule::deserialize(const std::string& json, google::protobuf::Message* message, std::string& error_msg)
{
	Deserializer deserializer;
	if (!deserializer.ReadMessage(json, message))
	{
		if (!deserializer.GetErrors().empty())
		{
			for (std::size_t i = 0; i < deserializer.GetErrors().size(); ++i)
			{
				error_msg += deserializer.GetErrors()[i];
				error_msg += "\n";
			}
		}
		else
		{
			error_msg += "deserialize error: unkown";
		}
