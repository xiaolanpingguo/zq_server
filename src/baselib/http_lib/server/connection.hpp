...
public:
	...
	void write_chunked_header(const std::string& mime, bool is_range = false) 
	{
...
private:
		...
		else 
		{
			...
			{ 
				...
				case content_type::chunked:
					...
					break;
				default:
					handle_nothing(bytes_transferred);
					break;
				}
			}
			else 
			{ 
				// 没有body，直接处理头即可，比如get请求，websocket之类的
	...
	void do_write() 
	{
		...
		boost::asio::async_write(socket_, std::move(buffers),
			[self = this->shared_from_this()](const boost::system::error_code& ec, std::size_t bytes_transferred) 
		{
	...
	content_type get_content_type() 
	{
		...
		auto content_str = req_.get_header_value("content-type");
		if (!content_str.empty())
		{
			if (content_str.find("application/x-www-form-urlencoded") != std::string::npos)
			{
				return content_type::urlencoded;
			}
			else if (content_str.find("multipart/form-data") != std::string::npos)
			{
				auto size = content_str.find("=");
				auto bd = content_str.substr(size + 1, content_str.length() - size);
				std::string boundary(bd.data(), bd.length());
				multipart_parser_.set_boundary("\r\n--" + std::move(boundary));
				return content_type::multipart;
			}
			else if (content_str.find("application/octet-stream") != std::string::npos)
			{
	...
	bool parse_multipart(size_t size, std::size_t length) 
	{
		...
		req_.set_part_data(std::string(req_.buffer(size), length));
		std::string multipart_body = req_.get_part_data();
	...
	void handle_nothing(size_t bytes_transferred)
	{
		response_back(status_type::not_found, "unknown error");
	}
	...
	void handle_body() 
	{
		if (req_.at_capacity()) 
		{
			response_back(status_type::not_found, "The body is too long, limitation is 3M");
			return;
		}
